...
- Say "I'm connected to your tools" — not "I have MCP integrations via the Codex API"
...
When the user types any of the following, read the full skill file at the path shown and execute it completely. This applies in Codex surfaces where slash commands are not natively registered. In Codex, skills are auto-discovered — this section is the universal fallback.

| Command | Execute |
|---|---|
| `/onboard` | `.agents/skills/open-frame-onboarding/SKILL.md` |
| `/audit` | `.agents/skills/audit/SKILL.md` |
| `/level-up` | `.agents/skills/level-up/SKILL.md` |
| `/explore` | `.agents/skills/explore/SKILL.md` |
| `/morning-brief` | `.agents/skills/morning-brief/SKILL.md` |
| `/roi-report` | `.agents/skills/roi-report/SKILL.md` |
...
These rules live in `.agents/rules/`. Read the relevant rule before acting when its signal appears.

- `business-scorecard` — watch for stalling domains, untracked metrics, and stale decisions.
- `change-management` — enforce ramp phase discipline before shipping or changing automations.
- `continuous-learning` — capture repeatable automation signals into `decisions/log.md` or `context/candidates.md`.
- `decision-follow-through` — surface open commitments when they become relevant.
- `opportunity-radar` — add specific automation candidates when repeated manual work shows up.
...
- `.agents/skills/` — Codex skills. Core skills live here and domain skills get added here.
- `.agents/rules/` — always-on operating rules for scorecard, learning, follow-through, change management, and opportunity spotting.
