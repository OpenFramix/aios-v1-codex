...
- **No files** → say: *"No AI history found — no problem. If you have a ChatGPT or Codex export, drop it in `references/ai-history/` and re-run `/onboard` to absorb it. For now we'll build context from the intake."* Continue.
...
**G — Surface gaps to the operator**
...
**Step 3.9 — Fill `AGENTS.md` placeholders**
...
After all context files are scaffolded and DESIGN.md is linted, run the Connection Sprint. This is done right now, during the setup session — not deferred to Day 2. the operator is at the computer. The client is present. Wire what can be wired today.
...
For each sprint target, use the cleanest Codex-native connection path available: a connected Codex tool first, then an API key + reference guide when no connector exists.

---

**Google Workspace (covers Domains: Communication, Calendar, Knowledge/files)**

- Connect Gmail, Calendar, and Drive through the available Codex tool connection.
...
- Update `connections.md` for each domain that now returns real data.
...
For any tool in the priority connections list that has a Codex connector available:
- Connect the tool through Codex.
- Test with a basic read query.

For tools without a Codex connector:
...
Set:
- `mechanism` = `mcp` (Codex connector), `key+ref` (API key + reference guide), or `script` (repo script hitting the API)
- `auth` = `connected`
...
9. **Connection Sprint happens on Day 1.** Wire at least 2-3 connections during the setup session. `.env` writes are expected and correct during the sprint. The goal is a connected AIOS before the operator walks out.
...
- **Brand variable test:** Change `{{operator_name}}` in AGENTS.md. Expected: closing screen reflects the new name with no other changes needed.
