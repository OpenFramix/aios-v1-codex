...
Runs the **OFX Architecture Audit** on the current Codex project. Reads (never writes) the project's operating manual, context files, skills, agents, connections, decisions, and runs history. Scores each of the five layers out of 20. Surfaces strengths and the top 3 leverage-weighted gaps with concrete next steps.
...
**Operating manual:** `AGENTS.md` (root), `AGENTS.local.md` (gitignored)
...
**Skills:** `.agents/skills/*/SKILL.md` — count + read frontmatter only
**Agents:** `.agents/agents/*.md` — count
**Connections:** `connections.md` (anywhere), `.env`, `.mcp.json`, connected Codex tools, API scripts in `scripts/`, reference guides in `references/`
...
**Interface:** Any mention of a dashboard URL, `dashboard/` folder, or interface project reference in AGENTS.md
**Recurring cadence:** documented Codex app automations, dated files in `runs/`, or skill names matching `morning-*`, `daily-*`, `weekly-*`, `monthly-*`
...
|---|---|---|
| AGENTS.md exists and is substantive (>200 words) | 4 | Read and count words |
...
| Criterion | Points | How to detect |
|---|---|---|
| 3+ skills installed | 8 | Count `.agents/skills/*/SKILL.md` |
...
| 1+ agent defined | 4 | `.agents/agents/*.md` ≥ 1 |
...
- AGENTS.md missing or thin: **3x** — no foundation
