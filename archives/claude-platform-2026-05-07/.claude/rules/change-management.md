...
| 2 | Guided | Runs automatically and produces a draft. Sam reviews and approves before the output goes anywhere. |
| 3 | Watched | Runs automatically. Output goes live. Sam spot-checks periodically. |
...
> *"Phase 1 — Training Wheels. Review this output before it goes anywhere. When this has run correctly 5+ times over 2+ weeks, Sam can advance it to Phase 2 by editing `ramp-phase: 1` → `ramp-phase: 2` in the frontmatter."*
...
Advancement requires Sam to manually edit the `ramp-phase` value in the automation's frontmatter file. The AIOS does not advance phases automatically under any circumstance — not based on run count, not based on time, not based on client request.

If a client asks to advance a phase, respond:
> *"Phase advancement is Sam's call — it requires confirming the automation has run correctly at least 5 times over at least 2 weeks. I'll flag this for the operator at the next `/level-up` session."*

---

## Advancement Criteria (for Sam's reference)
...
"Confirmed" means Sam reviewed the output and did not flag a problem. A run that produced wrong output, sent something it shouldn't have, or required manual correction does not count.
...
If Sam says yes, walk through the advancement criteria for each one and confirm both conditions are met before recommending the edit. If Sam says no, log the check in the session and move on.
...
A broken automation that fires without Sam's review is the fastest way to lose a client's trust. Training Wheels are not a delay — they're how reliable systems get built. Every automation that reaches Phase 3 or 4 earned it. That's what the retainer is worth.
