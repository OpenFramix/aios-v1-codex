...
If Sam says yes — walk through the advancement criteria (5+ confirmed runs, 2+ weeks) for each one. If both conditions are met, recommend the frontmatter edit and tracking file update. If Sam says no or `tracking/` is empty — move on.
...
If Sam wants to start with a captured item — skip directly to Phase 2 (Method) with that candidate. Mindset already happened in the background.
If Sam wants them factored in — weight them as top candidates during Phase 1. A real observed signal outranks a hypothetical from domains.md.
...
- **Remote** — uses only Claude Code's native tools (web search, file creation). Can run in the cloud, machine can be off.

This determines how the automation gets deployed and whether it needs to be scheduled locally or via a Claude Code routine.
...
This locks the client and Sam into Phase 1 of The Ramp on first build. Advances only by explicit edit after validation.
...
The `ramp-phase` frontmatter field is what the stale check and change-management rule read. Always use frontmatter for this field — never body markdown. Update it here whenever Sam advances the phase.
