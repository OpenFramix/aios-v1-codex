...
**This skill drives the Cadence layer score.** Scheduling it in `.claude/settings.json` is what earns the recurring trigger points in `/audit`. Without a scheduled skill, the AIOS only runs when asked. With a scheduled morning brief, it runs on its own.
...
Two options — use whichever fits the client's VPS setup:

**Option A — System cron (recommended for VPS installs):**
```bash
# Add to crontab: crontab -e
0 7 * * 1-5 cd /path/to/client-aios && claude -p "/morning-brief" --output-format stream-json >> /var/log/morning-brief.log 2>&1
```
Runs Monday–Friday at 7am server time. Adjust the cron expression for the client's timezone.

**Option B — Claude Code Routines:**
Open Claude Code → Routines (sidebar) → New Routine → set schedule and prompt `/morning-brief`.

Weekends optional — ask the client.
...
| Phase | What happens |
|---|---|
| 1 — Training Wheels | Sam runs it manually each morning, reviews the output, confirms it's accurate |
| 2 — Guided | Scheduled to run automatically, Sam reviews before acting on any flagged items |
| 3 — Watched | Runs automatically, Sam glances at it each morning, spot-checks for accuracy weekly |
...
- **Scheduling test.** Add the cron hook to settings.json. Expected: brief runs automatically at the scheduled time without manual trigger.
